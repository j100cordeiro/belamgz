...
SELECT
    ...
    numnota,
    codvend,
    ROUND(SUM(rebate_item), 2) AS vlr_rebate
FROM base
GROUP BY nunota, numnota, codvend
HAVING ROUND(SUM(rebate_item), 2) <> 0
ORDER BY vlr_rebate DESC;
...
SELECT ROUND(SUM(rebate_item), 2) AS total_rebate
FROM base;
